...
def generate_chart_code(dataframe):
    prompt = f"""
    You are an expert in data visualization. Given a pandas DataFrame with the following columns: {', '.join(dataframe.columns)}, generate the best charting code using Plotly. The code should produce an informative and visually appealing chart. Use white backgound to display chart and make all the elements of chart on white background visibly.
    ...
    full_prompt = [
        {"role": "system", "content": "You are an expert in data visualization using Plotly. Brand colour is purple, use majorly white and purple shades. give proper legends, title, and data axis. Make a 3D looking chart in 2D, that looks professional and super appealing. use valid hex color code as color id in code. Start python code with string '```python' and end with '```'"},
        {"role": "user", "content": prompt}
    ]
